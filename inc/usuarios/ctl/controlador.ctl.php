...
switch( $_REQUEST['acc'] ){
	...
	case "actualizar":
		$res = $db::table("usuarios")->where("id", $_REQUEST['idObjeto'])->update([  /*"password" => $_REQUEST['password'], */ "nombre" => $_REQUEST['nombre'],  "email" => $_REQUEST['email'],  "servidorSMTP" => $_REQUEST['servidorSMTP'],  "passwordSMTP" => $_REQUEST['passwordSMTP'],  "tema" => $_REQUEST['tema']  ]);
		if($res > 0){
			$response['ack'] = 200;
