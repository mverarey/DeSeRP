...
$script = <<<EOM
	...
	$('#frmActualizar').on('show.bs.modal', function (event) {    $("#cargando").show();    $("#cargandodiv").show();    var ids = $.map($("#tbldatab03f996214fba4a1d05a68b18fece8e71").bootstrapTable('getSelections'), function (row) {	return row.id });    var opc=ids[0];     if(opc>0){	$.ajax({ method: 'POST', dataType:'json', url: '/wsdl/{$this->os(2)}/obtenerobjetos/'+opc }).done(function( data ) { $("#cargando").hide(); $("#cargandodiv").hide();  $("#idObjeto").val(data.id);
     $("#dtxtnombre").val(data.nombre);  $("#dtxtemail").val(data.email);  $("#dtxtservidorSMTP").val(data.servidorSMTP);  $("#dtxtpasswordSMTP").val(data.passwordSMTP);  $("#dtxttema").val(data.tema); });  }else{ 	alert("Debe seleccionar usuario primero.");	$("#cargando").hide(); $("#cargandodiv").hide(); return false; } });

		 /* $("#dtxtpassword").val(data.password); */
	$("#btnActualizar").click(function(){
		$("#cargando").show();    $("#cargandodiv").show(); var valores = {}; var nombres = ["usuario","password","nombre","email","servidorSMTP","passwordSMTP","tema","fecha_creacion","activo"]; var campos = ["dtxtusuario","dtxtpassword","dtxtnombre","dtxtemail","dtxtservidorSMTP","dtxtpasswordSMTP","dtxttema","dtxtfecha_creacion","dtxtactivo"];  jQuery.each( campos, function( i , campo){ valores[nombres[i]] = $("#"+campo).val(); }); valores["idObjeto"] = $("#idObjeto").val(); valores["acc"] = "actualizar"; $.ajax({ type: 'POST', dataType:'json', data:valores, url: '/wsdl/{$this->os(2)}/controlador' }).done(function( data ){ if(data.ack == 200){ $.notify(data.msg, "success");$("#tbldatab03f996214fba4a1d05a68b18fece8e71").bootstrapTable('refresh'); $('#frmActualizar').modal('hide'); }else if(data.ack == 201){ $.notify(data.msg, "warn"); }else{ $.notify(data.msg, "error"); } }).fail(function() { alert( "error" ); $.notify(data.msg, "success"); }).always(function(){ $("#cargando").hide();    $("#cargandodiv").hide(); });
	});
